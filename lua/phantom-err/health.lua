...
function M.check()
  ...
  if vim.fn.has("nvim-0.8") == 1 then
    vim.health.ok(string.format("Neovim version %d.%d.%d (requires 0.8+)", nvim_version.major, nvim_version.minor, nvim_version.patch))
  else
  ...
  if go_parser_available then
    ...
    vim.health.info("Or manually: git clone https://github.com/tree-sitter/tree-sitter-go ~/.local/share/nvim/site/pack/packer/start/tree-sitter-go/")
  end
  ...
  if config_ok then
    ...
    if opts then
      ...
      vim.health.info(string.format("Current config: auto_enable=%s, fold_errors=%s, single_line_mode=%s", 
        tostring(opts.auto_enable), tostring(opts.fold_errors), opts.single_line_mode))
    else
  ...
  if parser_ok then
    ...
    if go_parser_available then
      ...
      if test_success and test_result then
        vim.health.ok(string.format("Parser functional test passed (found %d regular blocks, %d inline blocks, %d assignments)", 
          test_result.regular_blocks, test_result.inline_blocks, test_result.assignments))
      else
  ...
  if buf_count > 50 then
    vim.health.warn(string.format("High buffer count (%d) - consider closing unused buffers for better performance", buf_count))
  else
